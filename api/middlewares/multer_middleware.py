...
async def multer_middleware(request:Request,upload_file: UploadFile = None) -> str:
    logging.info(f"multer_middleware - entered with upload_file: {upload_file}")
    thread_id = request.cookies.get("thread_id")
    if not upload_file or not upload_file.filename:
        logging.info("multer_middleware - no file was uploaded. returning empty path.")
        return ""
    file_path = os.path.join(PUBLIC_TEMP_DIR,thread_id,upload_file.filename)
